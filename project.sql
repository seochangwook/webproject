...
create or replace procedure PRO_LOGIN_ENROLL(
  -- 매개변수 선언
  in_pro_type IN number, -- type = 1(로그인 유효검사), type = 2(학생등록), type = 3(아이디 중복검사), type = 4(학생정보 수정), type = 5(아이디 찾기), type = 6(비밀번호 찾기)
...
-- 커서정의, 사용할 변수 정의
v_result_col number;
v_result_searchid varchar2(150);
v_result_searchpassword varchar2(150);
BEGIN
  ...
  ELSIF in_pro_type = 4 THEN
    ...
    COMMIT;
  -- ID찾기
  ELSIF in_pro_type = 5 THEN
    SELECT student.stu_id INTO v_result_searchid FROM student WHERE student.stu_number = in_stu_number AND student.stu_name = in_stu_name; -- ID 찾기
    IF v_result_searchid = '' THEN
      out_result := '0';
    ELSE
      out_result := v_result_searchid;
    END IF;
  ELSIF in_pro_type = 6 THEN
    SELECT student.stu_password INTO v_result_searchpassword FROM student WHERE student.stu_id = in_stu_id;
    IF v_result_searchpassword = '' THEN
      out_result := '0';
    ELSE
      out_result := v_result_searchpassword;
    END IF;
  ELSE
    out_result := '0';
    COMMIT;
...
SELECT student.stu_id FROM student WHERE student.stu_number = 201158102 AND student.stu_name = '서창욱'; -- ID 찾기
SELECT student.stu_password FROM student WHERE student.stu_id = 'scw0531'; -- PW찾기
-------------------------------------------
--# 수강신청 관련 프로시저
...
-------------------------------------------
--# 나의 수강신청 목록 출력
SELECT stu_number, c.c_number, c.c_name, c.c_date_time, c.c_grade, e.c_memo FROM course c, enroll e WHERE c.c_number = e.c_number AND e.STU_NUMBER= 201158102;
SELECT stu_number, c.c_number, c.c_name, c.c_date_time, c.c_grade, e.c_memo FROM course c, enroll e WHERE c.c_number = e.c_number AND e.STU_NUMBER= 201158102 AND c.c_number = 12;